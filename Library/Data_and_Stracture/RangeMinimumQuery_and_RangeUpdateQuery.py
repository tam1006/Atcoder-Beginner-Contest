...
class RangeMinimumUpdateQuery:
    ...
    def from_list(cls, A: List[T], identity: T):
        ...
        for i in range(N0-1, 0, -1):
            value[i] = min(value[i*2], value[i*2+1])
    ...
    def update(self, l, r, x):
        ...
        *ids, = self.gindex(l, r)
        ...
        for i in ids:
            self.value[i-1] = min(self.value[i*2-1], self.value[i*2])
    ...
    def query(self, l, r):
        ...
        while L < R:
            if R & 1:
                R -= 1
                s = min(s, self.value[R-1])

            if L & 1:
                s = min(s, self.value[L-1])
                L += 1
...
RMUQ = RangeMinimumUpdateQuery.new(N, (1 << 31) - 1)
